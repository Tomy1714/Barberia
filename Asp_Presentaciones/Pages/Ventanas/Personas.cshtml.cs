...
{
    public class PersonasModel : PaginaBase
    {
        private readonly PersonasPresentacion _negocio = new();

        public List<Personas> Lista { get; private set; } = new();
        public List<FilaAuditoria> Auditoria { get; private set; } = new();

        [BindProperty] public Personas Item { get; set; } = new();
        ...
        public async Task<IActionResult> OnPostGuardarAsync()
        {
            ...
            try
            {
                if (Item.IdPersona == 0) await _negocio.Guardar(Item, RolActual);
        ...
        public IActionResult OnPostEliminar()
        {
            var redir = ValidarAcceso("Administrador");
            if (redir != null) return redir;
            try { _negocio.Eliminar(Item, RolActual); Mensaje = "Persona eliminada."; }
            catch (Exception ex) { ErrorMsg = ex.Message; }
            return RedirectToPage();
        }
        ...
        private void Cargar()
        {
            try
            {
                ...
                Auditoria = (_negocio.ConsultarAuditoria(RolActual) ?? new())
                    ...
                    {
                        IdAuditoria = a.IdAuditoria, IdReferencia = a.IdPersona,
